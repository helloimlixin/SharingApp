 ...
 * item's id.
 * Note: invisible EditText is used to setError for status. For whatever reason we cannot .setError
 * to the status Switch so instead an error is set to an "invisible" EditText.
 */
public class EditItemActivity extends AppCompatActivity {
    ...
    public void toggleSwitch(View view){
        if (status.isChecked()) {
            // If the item is previously borrowed, the switch will be toggled to available.
            borrower_spinner.setVisibility(View.GONE);
            borrower_tv.setVisibility(View.GONE);
